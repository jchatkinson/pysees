...
export interface CommandSchema {
  ...
  args: ArgDef[]
  optional: ArgDef[]
  create: (values: Record<string, unknown>, model: ModelState) => Command
}
...
export const V1_COMMAND_SCHEMAS: CommandSchema[] = [
  {
    ...
    label: 'Node',
    category: 'model',
    args: [{ kind: 'vec', name: 'coords', label: 'Coordinates', length: 'ndm', defaultValue: [0, 0, 0] }],
    optional: [],
    create: (values, model) => ({ type: 'ADD_NODE', id: model.nextNodeId, coords: nums(values.coords) }),
  },
  {
    cmd: 'FIX',
    fn: 'fix',
    ...
    category: 'model',
    args: [
      { kind: 'int', name: 'nodeId', label: 'Node ID', defaultValue: 1 },
      { kind: 'vec', name: 'dofs', label: 'DOF Fix Flags (0/1)', length: 'ndf', defaultValue: [1, 1, 1, 0, 0, 0] },
    ],
    optional: [],
    create: (values) => {
  ...
  },
  {
    ...
    args: [
      { kind: 'int', name: 'nodeId', label: 'Node ID', defaultValue: 1 },
      { kind: 'vec', name: 'values', label: 'Load Values', length: 'ndf', defaultValue: [0, 0, 0, 0, 0, 0] },
    ],
    optional: [],
  ...
  },
  {
    ...
    category: 'model',
    args: [
      { kind: 'choice', name: 'eleType', label: 'Element Type', options: ['Truss', 'ElasticBeamColumn'], yields: {}, defaultValue: 'Truss' },
      { kind: 'vec', name: 'nodes', label: 'Node IDs', length: 2, defaultValue: [1, 2] },
    ],
    optional: [],
    create: (values, model) => ({
      type: 'ADD_ELEMENT',
